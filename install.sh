...
# Overwrite a few of the polargraphcontroller pde files with modified ones
# that add a "Draw Tesla" button that eliminiates the need to do some manual steps.
cp -f $BUILD_DIRECTORY/pde/*.pde $HOMEDIR/sketchbook/polargraphcontroller/

# Copy default Tesla image into the sketchbook.
cp -f $BUILD_DIRECTORY/Em_Tesla_INCH.svg $HOMEDIR/sketchbook/polargraphcontroller/
